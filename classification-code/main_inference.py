        ...
        if step_type=='test':
            ...
            with open(self.output_file_name,'a',encoding='utf-8') as file_handler:
                for idx in range(len(batch_input_sentences)):
                    output_dict=dict()
                    output_dict['sentence'] = batch_input_sentences[idx]
                    output_dict['predict_label'] = np.int(self.predict_labels[-1][idx])
                    output_dict['true_label'] = np.int(self.labels[-1][idx])
                    output_dict['logit_0']=np.float(logit_0[idx].item())
                    output_dict['logit_1']=np.float(logit_1[idx].item())
                    output_json_string=json.dumps(output_dict, ensure_ascii=False)
                    file_handler.write(output_json_string+"\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_file_path', type=str, required=True, 
                        help='specify the checkpoint path while generating inference')
    ...
    logger_args = {
        'project': 'ALL-LANG-WNC-INFOXLM',    # first create a project on weight & bias with local account
    ...
    ckpt_path=args.checkpoint_file_path
    ...
    # Command to run --> python3 finetune_multilingual_encoder_models/main_inference.py --dataset_path /scratch/user/dataset_models_bias --gpus 1 --fp16 1 --lang mr --batch_size 8 --learning_rate 1e-6 --weight_decay 0.001 --epochs 10 --patience 10 --output_file_path model_output_mr.jsonl --checkpoint_file_path /scratch/user/mr-google-muril-base-cased-10-1e-06_epoch=9.ckpt --model_name google/muril-base-cased
